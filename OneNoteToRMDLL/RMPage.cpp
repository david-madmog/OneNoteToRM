...
static size_t ReadHelper(concurrency::streams::istream stream, void * dest, size_t size)
{
    concurrency::streams::rawptr_buffer<char8_t> Buffer((char8_t *)dest, size);
    pplx::task<size_t> ReadBytesTask = stream.read(Buffer, size);
    size_t ReadBytes = ReadBytesTask.get();
...
void RMPage::Load(concurrency::streams::istream Doc){
    ...
    NumRead = ReadHelper(Doc, &FM, sizeof(FM));
//        DoLog((const char*)&FM);
    ...
    while (NumRead > 0) {
        ...
        NumRead = ReadHelper(Doc, & BH, sizeof(BH));
